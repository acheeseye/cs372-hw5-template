...
double Shape::get_xpos() const { return m_xpos; }
double Shape::get_ypos() const { return m_ypos; }

void Shape::set_width(double width) { m_width = width; }
void Shape::set_height(double height) { m_height = height; }
void Shape::set_xpos(double x) { m_xpos = x; }
void Shape::set_ypos(double y) { m_ypos = y; }

void Shape::generate_postscript_file(const std::string & file_name) const
{
	double margin = 0;
	std::string output_postscript = std::to_string(get_width() / 2 + margin) + " " +
		std::to_string(get_height() / 2 + margin) + " translate\n\n" +
		to_postscript() + "\nshowpage";
...
std::string Shape::g_save() const
{
	return "gsave\n";
}

std::string Shape::g_restore() const
{
	return "grestore\n";
}

Compound::Compound(const ShapeCollection& shapes)
	:Shape(0, 0), m_shapes(shapes)
{
	m_translate_points.resize(m_shapes.size());
}

std::string Compound::to_postscript() const
{
	string ret_str;
...
Compound::ShapeCollection Compound::get_shapes() const
{
	return m_shapes;
}

CompoundLayered::CompoundLayered(const ShapeCollection& shapes)
	:Compound(shapes)
{
	CompoundLayered::generate_translate_points();
	for (auto& m_shape : m_shapes)
	{
		if(get_width() < m_shape->get_width())
			set_width(m_shape->get_width());
		if(get_height() < m_shape->get_height())
			set_height(m_shape->get_height());
	}
}

std::string CompoundLayered::translate(const int nil) const
{
	return "";
}

void CompoundLayered::generate_translate_points()
{}

CompoundVertical::CompoundVertical(const ShapeCollection& shapes)
	:Compound(shapes) 
{
	...
	for (auto& m_shape : m_shapes)
	{
		if(get_width() < m_shape->get_width())
			set_width(m_shape->get_width());
		total_height += m_shape->get_height();
	}
	set_height(total_height);
	CompoundVertical::generate_translate_points();
}

std::string CompoundVertical::translate(const int index) const 
{
	return "0 " + to_string(m_translate_points[index]) + " translate\n";
}

void CompoundVertical::generate_translate_points()
{
	double total_height_drawn = 0;
	for (unsigned int i = 0; i < m_shapes.size(); ++i)
	{
		m_translate_points[i] = -get_height() / 2 +
			m_shapes[i]->get_height() / 2 +
			total_height_drawn;
...
CompoundHorizontal::CompoundHorizontal(const ShapeCollection& shapes)
	:Compound(shapes)
{
	...
	for (auto& m_shape : m_shapes)
	{
		if(get_height() < m_shape->get_height())
			set_height(m_shape->get_height());
		total_width += m_shape->get_width();
	}
	set_width(total_width);
	CompoundHorizontal::generate_translate_points();
}

std::string CompoundHorizontal::translate(const int index) const
{
	return to_string(m_translate_points[index]) + " 0 translate\n";
}

void CompoundHorizontal::generate_translate_points()
{
	double total_width_drawn = 0;
	for (unsigned int i = 0; i < m_shapes.size(); ++i)
	{
		m_translate_points[i] = -get_width() / 2 +
			m_shapes[i]->get_width() / 2 +
			total_width_drawn;
...
std::string Vertical::to_postscript() const
{
	std::string outputString = "";
	double total_height_drawn = 0;
	for(unsigned int i = 0; i < m_shapes.size(); ++i)
